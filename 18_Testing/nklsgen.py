...
def dict_to_array(ls_dict):
    base = len(set([a[0] for a in list(ls_dict.keys())]))
    dims = len(list(list(ls_dict.keys())[0]))
    shape = tuple([base for _ in range(dims)])
    data = np.zeros(shape)
    for loc, val in list(np.ndenumerate(data)):
        data[loc] = ls_dict[loc]
    return data
    

def smooth_landscape(landscape, stride=1, ratio=0, norm=True):
    
    kernel = np.ones(tuple([1+stride*2 for _ in range(len(landscape.shape))]))
    if ratio != 0:
        mult = (ratio/(1-ratio))
        elseval = kernel.sum() - 1
        kernel[tuple([1 for _ in range(len(landscape.shape))])] = elseval*mult
    kernel = kernel/kernel.sum()    
    smooth_data = convolve(landscape, kernel)
...
def nk_landscapes(N, K, num_of_ls=1, identical=False, simple_output=True, base=2):
    ...
    for ls in list(range(num_of_ls)):
        ...
        int_matrix, proto_landscape, ls_key = prep(N, K, base)
...
def generate_landscapes(N, K, num_of_ls=1, base=2, smooth=[0,1,0], preprocessed=True):
    """
    ...
    landscapes_ = nk_landscapes(N, K, num_of_ls, identical=False, simple_output=True, base=base)
    ...
    for ls in landscapes_:
        ...
        keys = [tuple([int(a) for a in code]) for code in keys_.tolist()]
        values = [round(val[0], 10) for val in values_]
        landscapes.append(dict_to_array(normalize_landscape(dict(zip(keys, values)))))

    if smooth[0] != 0:
        smooth_landscapes = []
        for ls in landscapes:
            for _ in range(smooth[0]):
                smooth_landscapes.append(smooth_landscape(ls, stride=smooth[1], ratio=smooth[2]))
        landscapes = smooth_landscapes
