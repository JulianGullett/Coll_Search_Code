...
class Agent():
    ...
    def __init__(self, landscape, points = 1, distance_cost = 0, memory = None, search_hist = None, neighbors = None, myopic=False):
        ...
        # Memory initialization (if none, memory is infinite)
        if memory == None:
            self.memory = 10**10
        else:
            self.memory = memory
        
        # Is agent myopic?  (if so, search cost is set unreasonably high)
        if myopic:
            self.distance_cost = 10**6
        else:
            self.distance_cost = distance_cost
        
        # Initialize search history data structure (prob better to make this something else)
        ## random_start() selects a number of points to begin with (# == points)
        if search_hist is None:
            self.search_hist = pd.DataFrame(columns=['Location', 'Value', 'Step'])
            self.random_start(points)
        
        # Current position (don't think this is needed anymore)
        self.current_position = self.search_hist['Location'].iloc[-1]
        
        # Initialize neighbor list (is historical)
    ...
    def random_start(self, points):
        """
        Choose random points on landscape 
